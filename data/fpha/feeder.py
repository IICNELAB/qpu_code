...
from data.fpha.fpha_info import LABEL_NAMES
...
class Feeder(torch.utils.data.Dataset):
    ...
    def load_data(self, mmap):
        ...
        # load data
        if mmap:
            self.data = np.load(self.data_path, mmap_mode='r')  # (N, C, T, V)
        else:
    ...
    def __getitem__(self, index):
        data_numpy = np.array(self.data[index].take(self.ind[index], axis=1))  # C, T, V        
