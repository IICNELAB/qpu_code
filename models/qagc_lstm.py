...
class AGC_LSTM(nn.Module):
    ...
    def forward(self, x):
        """
        x: (batch, in_channels, num_frames, num_joints, num_body)
           (B,C,T,V,M)
        """
        # FC
        ...
        if self.pa>0: 
            x_p = x[...,self.parent,:].reshape(num_frames, batch_size * num_joints * num_body, -1, 1)
            x_c = x.reshape(num_frames, batch_size * num_joints * num_body, -1, 1)
            if self.pa==2: 
                x_pp = x[...,grandpa,:].reshape(num_frames, batch_size * num_joints * num_body, -1, 1)
                x = torch.cat([x_c,x_p,x_pp],-1).reshape(num_frames, batch_size * num_joints * num_body, -1)
            else:
                x = torch.cat([x_c,x_p],-1).reshape(num_frames, batch_size * num_joints * num_body, -1)
        else:
...
if __name__ == "__main__":
    # in_channels, num_joints, num_frames, num_cls, config
    net = AGC_LSTM(5,25,48,10,None).to('cuda:0')
    # batch, in_channels, num_frames, num_joints
