...
from pointnet2.utils import qpu_layers, qpu_ops
...
class PointnetSAModuleMSGQPU(nn.Module):
    def __init__(self,  npoint, radii, nsamples, mlps, use_xyz=False):
        super(PointnetSAModuleMSGQPU, self).__init__()
        self.npoint = npoint
        self.mlps = nn.ModuleList()
        self.groupers = nn.ModuleList()
        for i in range(len(radii)):
...
class PointnetSAModuleQPU(PointnetSAModuleMSGQPU):
    r"""Pointnet set abstrction layer
    ...
    def __init__(
        ...
        # type: (PointnetSAModule, List[int], int, float, int, bool, bool) -> None
        super(PointnetSAModuleQPU, self).__init__(
            mlps=[mlp],
            npoint=npoint,
            radii=[radius],
            nsamples=[nsample],
